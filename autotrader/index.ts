...
class AutoTrader {
  ...
  private async enterTrade(result: ScannerResult, equity: number, openTrades: Awaited<ReturnType<typeof fetchOandaOpenTrades>>) {
    ...
    const signalKey = `${result.instrument}:${this.config.mode}:${bias}:${result.updatedAt}:${result.strategyVersion}:${result.selectedStrategy?.id ?? "trend-continuation"}`;
