...
export type StrategyId = "trend-pullback" | "range-breakout" | "liquidity-reclaim" | "imbalance-continuation" | "discovered-rules";
export type RuleFeature = 'distance' | 'previousDistance' | 'trend' | 'momentum' | 'aboveHigh' | 'aboveLow' | 'lowSweep' | 'highSweep' | 'body' | 'rangeSize';
export type RulePredicate = Readonly<{feature:RuleFeature;op:'gt'|'lt';value:number}>;
export type RuleSet = Readonly<{long:readonly RulePredicate[];short:readonly RulePredicate[]}>;
export type Strategy = Readonly<{ id: StrategyId; name: string; version: string; parameters: Readonly<{ lookback: number; stopAtr: number; targetR: number; maxBars: number }>; markets: readonly string[]; timeframes: readonly string[]; preferredRegimes: readonly Regime[]; avoidRegimes: readonly Regime[]; activeRegimes?: readonly Regime[]; ruleSet?: RuleSet; entryRules: string; exitRules: string; invalidation: string; lifecycle: "research" | "paper" }>;
