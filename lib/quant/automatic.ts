...
function parseJson<T>(value:string|null|undefined):T|null { if(!value) return null; try { const parsed = JSON.parse(value); return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as T : null; } catch { return null; } }
...
export async function automaticStatus() {
 ...
 return {enabled:Boolean(config?.enabled),intervalMinutes:60,driver:'Checks run while Foresight is open or when the connected background worker sends a heartbeat. A scheduled Codex task can also trigger checks while the app is running.',jobs:rows.results.map(r=>({instrument:r.instrument,nextDueAt:r.next_due_at,running:r.lease_until>Date.now(),completedAt:r.completed_at,error:r.last_error,result:parseJson<ReturnType<typeof discoveryObservation>>(r.result_json)}))};
}
...
export async function claimResearch(now=Date.now()) {
 ...
 const row=await db.prepare("UPDATE quant_automation_jobs SET lease_until=?,lease_token=? WHERE instrument=(SELECT instrument FROM quant_automation_jobs WHERE (next_due_at<=? OR (result_json IS NOT NULL AND json_extract(CASE WHEN json_valid(result_json) THEN result_json ELSE '{}' END,'$.discovery') IS NULL AND last_error IS NULL)) AND lease_until<=? ORDER BY next_due_at,instrument LIMIT 1) AND lease_until<=? AND EXISTS (SELECT 1 FROM quant_automation_settings WHERE id='primary' AND enabled=1) RETURNING instrument").bind(now+300000,token,now,now,now).first<{instrument:string}>();
...
export async function runAutomaticResearch() {
 ...
 try {
  ...
  if (bars.length < 800) throw new Error('Automatic research needs at least 800 completed prices.');
  ...
  const reuse=previous?.discovery?.grammarVersion==='grammar-v1'&&Array.isArray(previous.discovery.finalists)&&Array.isArray(previous.discovery.candidates)&&previous.discovery.instrument===claim.instrument&&(now-(previous.discoveredAt??0)<86400000||bars.at(-1)!.closeTime<=previous.discovery.to);
