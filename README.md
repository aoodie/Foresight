...
This project runs on Vinext with Cloudflare Workers and D1. The hosted version
uses the `DB` D1 binding declared in `.openai/hosting.json`; a VPS deployment
must provide an equivalent Cloudflare-compatible runtime and database binding.
Do not commit `.env*` files or broker/API credentials.
