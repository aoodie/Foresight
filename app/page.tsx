    ...
    <main className="min-h-screen bg-[#07100f] text-[#e8f3ee] selection:bg-[#a4ffcf] selection:text-[#07100f]">
      ...
      <div className="mx-auto max-w-[1460px] px-5 py-8 lg:px-10">
        ...
        {scanner?.results.length ? <section className="mb-5">
          <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs tracking-[.14em] text-[#8aa29a]">DAILY AI ANALYSIS CARDS</p><h2 className="mt-1 text-xl">Three strategies explained in plain English</h2><p className="mt-1 text-xs text-[#71887f]">OpenAI combines the four-hour market reading with LuxAlgo research and may recommend waiting.</p></div><Button onClick={() => void generateAiStrategies(scanner.results, scanner.generatedAt)} disabled={!aiConnected || aiLoading} className="bg-[#a4ffcf] text-[#07100f] hover:bg-[#d0ffe1]"><BrainCircuit className={aiLoading ? "animate-spin" : ""}/>{aiLoading ? "Analysing top 3…" : aiConnected ? "Regenerate AI plans" : "Add AI key in Settings"}</Button></div>
