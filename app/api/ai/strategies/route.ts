...
export async function POST(request: Request) {
  ...
  try {
    if (!body?.force) {
      ...
      if (cached) {
        await writeSystemLog({ category: "ai", event: "analysis.cache_hit", message: "Reused the captured AI strategy analysis; no LLM request was made.", details: { mode, instruments: markets.map((market) => (market as { instrument: string }).instrument), model: connection.model, baseUrl: connection.baseUrl, decisionId: cached.decisionId, fetchedAt: cached.fetchedAt } });
    ...
    await writeSystemLog({ category: "ai", event: result.cacheHit ? "analysis.cache_hit" : "analysis.completed", message: result.cacheHit ? "Reused an in-flight or captured AI strategy analysis; no additional LLM request was made." : `LLM generated ${output.strategies.length} strategy plan(s).`, details: { mode, instruments: markets.map((market) => (market as { instrument: string }).instrument), model: connection.model, baseUrl: connection.baseUrl, decisionId: result.decisionId, cacheHit: result.cacheHit, usage: result.usage } });
