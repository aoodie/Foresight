...
export async function POST(request: Request) {
  ...
  if (mode === "live" && !body.confirmLive) return NextResponse.json({ error: "Live execution is locked. Confirm live trading explicitly after reviewing the risk controls." }, { status: 409 });
  const connection = await getOandaToken();
  if (mode === "live" && !connection?.accountId) return NextResponse.json({ error: "Connect an OANDA account with an account number first." }, { status: 503 });
  ...
  try {
    const journalId = await createJournalEntry({
      ...
      status: mode === "paper" ? "simulated" : "submitted",
      ...
      openedAt: mode === "paper" ? new Date().toISOString() : null,
    });
    ...
    if (mode === "paper") return NextResponse.json({ mode: "paper", status: "simulated", orderId: `paper-${Date.now()}`, units: body.units, journalId });
