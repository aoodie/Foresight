...
type RecordRow = {
  ...
  lots: number | null; sizeLockScope: string | null; sizeLockPeriod: string | null; riskSafeUnits: number | null;
...
type Report = {
  connected: boolean;
  brokerError?: string;
  reconciliation?: { checked: number; activityUpdates: number; closedUpdates: number };
  generatedAt: string;
...
export default function ValidationPage() {
  ...
  return <main className="min-h-screen bg-[#07100f] text-[#e8f3ee] selection:bg-[#a4ffcf] selection:text-[#07100f]">
    ...
    <div className="mx-auto max-w-[1460px] px-5 py-8 lg:px-10">
      ...
      {report?.reconciliation && (report.reconciliation.activityUpdates > 0 || report.reconciliation.closedUpdates > 0) && <div className="mt-5 flex gap-3 rounded-xl border border-[#89f6bf]/20 bg-[#89f6bf]/[.05] p-4 text-sm text-[#bfffdc]"><CheckCircle2 size={18} className="mt-0.5 shrink-0" /><p><strong>Journal repaired from OANDA.</strong> {report.reconciliation.closedUpdates} closed trade{report.reconciliation.closedUpdates === 1 ? "" : "s"} and {report.reconciliation.activityUpdates} broker activity change{report.reconciliation.activityUpdates === 1 ? "" : "s"} were recorded.</p></div>}
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <Metric label="Average result per trade" value={signed(report?.metrics.expectancyR ?? null, 2, "R")} detail="Average outcome per planned unit of risk" tone={(report?.metrics.expectancyR ?? 0) > 0 ? "good" : "risk"} />
      ...
      {selected && <section className="mt-6 overflow-hidden rounded-2xl border border-[#a4ffcf]/15 bg-[#0c1916]">
        ...
        <div className="grid gap-6 p-5 sm:p-6 xl:grid-cols-[1.35fr_.65fr]">
          ...
          <aside className="space-y-4">
            <ReplayCard icon={<Scale size={16} />} title="Size, risk and execution"><ReplayRow label="Fixed units" value={selected.units === null ? "—" : Math.abs(selected.units).toLocaleString()} /><ReplayRow label="Standard lots" value={selected.lots === null ? "N/A for this CFD" : selected.lots.toFixed(2)} /><ReplayRow label="Size lock" value={selected.sizeLockScope ? `${selected.sizeLockScope}${selected.sizeLockPeriod ? ` · ${selected.sizeLockPeriod}` : ""}` : "Legacy / manual"} /><ReplayRow label="Planned cash risk" value={selected.riskAmount === null ? "—" : money(-selected.riskAmount, report?.source.currency ?? null)} /><ReplayRow label="Planned entry" value={price(selected.plannedEntry, selected.instrument)} /><ReplayRow label="Actual entry" value={price(selected.actualEntry, selected.instrument)} /><ReplayRow label="Actual exit" value={price(selected.actualExit, selected.instrument)} /><ReplayRow label="Entry slippage" value={signed(selected.slippagePips, 2, " pips")} /><ReplayRow label="Planned reward" value={signed(selected.plannedR, 2, "R")} /><ReplayRow label="Realised outcome" value={signed(selected.realisedR, 2, "R")} /></ReplayCard>
            <ReplayCard icon={<FlaskConical size={16} />} title="Decision record"><Explanation label="Why the trade was opened" text={selected.thesis} fallback="The reason for opening this trade was not recorded." /><Explanation label="Evidence" text={selected.evidence} fallback="No supporting evidence was recorded." /><Explanation label="When the idea fails" text={selected.invalidation} fallback="No rule for when the idea fails was recorded." /></ReplayCard>
          </aside>
        </div>
      </section>}
