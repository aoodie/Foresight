"use client";
import { formatPositionSize } from "@/lib/trade-risk";
    ...
    <main className="min-h-screen bg-[#07100f] text-[#e8f3ee]">
      <header className="mx-auto flex max-w-[1460px] flex-wrap items-center justify-between gap-4 border-b border-white/10 px-5 py-5 lg:px-10">
        ...
        <nav className="flex flex-wrap gap-2 text-xs">
          ...
          <Link href="/validation" className="rounded-md px-3 py-2 text-[#a9bdb6] hover:bg-white/5">Record checks</Link>
      ...
      <div className="mx-auto max-w-[1460px] px-5 py-8 lg:px-10">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs tracking-[.16em] text-[#8aa29a]">YOUR TRADE HISTORY</p>
        ...
        <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
          <section className="rounded-2xl border border-white/10 bg-[#0c1916] p-5">
            ...
            <form onSubmit={create} className="mt-5 space-y-3">
              <label className="block text-xs text-[#a9bdb6]">Market<select className={inputClass + " mt-1 w-full"} value={form.instrument} onChange={(event) => setForm({ ...form, instrument: event.target.value })}>{["EUR_USD", "GBP_USD", "USD_JPY", "USD_CHF", "AUD_USD", "NZD_USD", "USD_CAD", "EUR_GBP", "EUR_JPY", "GBP_JPY", "XAU_USD", "US30_USD"].map((value) => <option key={value} value={value}>{value.replace("_", " / ")}</option>)}</select></label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-xs text-[#a9bdb6]">Direction<select className={inputClass + " mt-1 w-full"} value={form.direction} onChange={(event) => setForm({ ...form, direction: event.target.value })}><option value="long">Buy — expecting a rise</option><option value="short">Sell — expecting a fall</option></select></label>
                <label className="block text-xs text-[#a9bdb6]">Trading style<select className={inputClass + " mt-1 w-full"} value={form.style} onChange={(event) => setForm({ ...form, style: event.target.value })}><option value="scalping">Very short trades</option><option value="intraday">Within the day</option><option value="swing">Several days</option></select></label>
              </div>
              <label className="block text-xs text-[#a9bdb6]">Strategy name<input className={inputClass + " mt-1 w-full"} value={form.strategyName} onChange={(event) => setForm({ ...form, strategyName: event.target.value })} placeholder="e.g. Recovery after a failed breakout" /></label>
              <div className="grid grid-cols-2 gap-3">
              ...
              <label className="block text-xs text-[#a9bdb6]">Notes<textarea className={inputClass + " mt-1 h-24 w-full py-2"} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Why you would enter, what supports the idea, and when it would fail" /></label>
          ...
          <section className="rounded-2xl border border-white/10 bg-[#0c1916] p-5">
            ...
            <div className="mt-5 space-y-2">
              {entries.length ? entries.map((entry) => { const metadata = metadataFor(entry); return <button key={entry.id} type="button" onClick={() => { setSelectedId(entry.id); setResult({ status: entry.status === "planned" ? "open" : resultStatus(entry), pnl: entry.pnl == null ? "" : String(entry.pnl), notes: typeof entry.notes === "string" ? entry.notes : "" }); }} className={(selectedId === entry.id ? "border-[#a4ffcf]/40 bg-[#a4ffcf]/[.08]" : "border-white/10 bg-[#10221d] hover:border-white/20") + " w-full rounded-xl border p-4 text-left"}>
                <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-medium">{entry.instrument.replace("_", " / ")} · {entry.direction}</p><p className="mt-1 text-xs text-[#8aa29a]">{entry.strategy_name || "Strategy not recorded"} · {entry.strategyVersion || "Version not recorded"} · {entry.style}</p></div><span className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase text-[#a4ffcf]">{entry.status}</span></div>
                <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs sm:grid-cols-4"><span className="text-[#71887f]">Entry<strong className="ml-1 text-[#d9e8e1]">{entry.entry_price ?? "—"}</strong></span><span className="text-[#71887f]">Stop<strong className="ml-1 text-[#d9e8e1]">{entry.stop_loss ?? "—"}</strong></span><span className="text-[#71887f]">Target<strong className="ml-1 text-[#d9e8e1]">{entry.take_profit_1 ?? "—"}</strong></span><span className="text-[#71887f]">Size<strong className="ml-1 text-[#d9e8e1]">{formatPositionSize(entry.instrument, entry.units == null ? null : Number(entry.units))}</strong></span><span className="text-[#71887f]">Risk<strong className="ml-1 text-[#d9e8e1]">{entry.risk_percent == null ? "—" : `${entry.risk_percent}%`}</strong></span><span className="text-[#71887f]">Profit / loss<strong className={(Number(entry.pnl) >= 0 ? "text-[#89f6bf]" : "text-rose-200") + " ml-1"}>{entry.pnl ?? "—"}</strong></span><span className="text-[#71887f]">Exit<strong className="ml-1 text-[#d9e8e1]">{metadata.closePrice ?? "—"}</strong></span><span className="text-[#71887f]">Reason<strong className="ml-1 text-[#d9e8e1]">{entry.exitReason ?? metadata.closeReason ?? (entry.status === "open" ? "Still open" : "Not recorded")}</strong></span></div>
                ...
                <div className="mt-3 rounded-lg border border-white/10 p-3 text-xs leading-6 text-[#c2d5ca]"><p><strong>Market at entry:</strong> {entry.marketCondition ?? "Not recorded at entry"}</p><p><strong>Original reasoning:</strong> {entry.entryReasoning ?? entry.thesis ?? "Not captured. This cannot be reliably reconstructed after the trade."}</p><p><strong>Result relative to risk:</strong> {entry.rMultiple == null ? "Original risk or realised result is missing" : `${Number(entry.rMultiple).toFixed(2)} times the original planned risk`}</p></div>
